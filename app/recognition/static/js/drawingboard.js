...
DrawingBoard.Board.prototype = {
	...
	reset: function(opts) {
		...
		this.history.clear();  // FIXME: monkey patch: only save the history of current conversation
	...
	initialize: function() {
		...
		if (this.opts.back) {
			...
			this.$el.on('click', '.drawing-board-control-navigation-back', $.proxy(function(e) {
				...
				this.board.ev.trigger('navigation:back');  // FIXME: monkey patch
				e.preventDefault();
			}, this));
		...
		if (this.opts.forward) {
			...
			this.$el.on('click', '.drawing-board-control-navigation-forward', $.proxy(function(e) {
				...
				this.board.ev.trigger('navigation:forward');  // FIXME: monkey patch
				e.preventDefault();
			}, this));
